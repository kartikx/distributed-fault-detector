...
func printMessage(direction string, message Message, nodeId string) {
	...
	case PING:
		...
		err := json.Unmarshal([]byte(message.Data), &messages)
		if err != nil {
			fmt.Printf("Failed to unmarshal PING submessages")
			return
		}

		fmt.Printf("Submessages vvvvv\n")
		for _, subMessage := range messages {
			printMessage(direction, subMessage, nodeId)
		}
		fmt.Printf("Submessages ^^^^^\n")
	case ACK:
	...
	case JOIN:
		fmt.Printf("JOIN message with %s\n", message.Data)

	case LEAVE:
		fmt.Printf("LEAVE message with %s\n", message.Data)

	case FAIL:
		fmt.Printf("FAIL message with %s\n", message.Data)

	case HELLO:
		fmt.Printf("HELLO message with %s\n", message.Data)

	case SUSPECT:
		fmt.Printf("SUSPECT message with %s\n", message.Data)

	case ALIVE:
		fmt.Printf("ALIVE message with %s\n", message.Data)

	case SUSPECT_MODE:
		fmt.Printf("SUSPECT_MODE message with %s\n", message.Data)

	default:
		fmt.Printf("********Trying to print unknown message type**********")
	}
	fmt.Printf("---------\n")
}
